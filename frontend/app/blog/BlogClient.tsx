...
export default function BlogClient({ initialBlogs, isError = false }: BlogClientProps) {
  ...
  return (
    <div className="relative min-h-[85vh] py-20 px-6 max-w-7xl mx-auto">
      ...
      {/* Search Input bar */}
      <ScrollReveal delayClass="animation-delay-100" className="w-full max-w-md">
        ...
        {isError ? (
          ...
          filteredBlogs.map((blog, index) => (
            <ScrollReveal key={blog.id} delayClass={`animation-delay-${(index % 3) * 100}`} className="w-full flex">
              <Link 
                href={`/blog/${blog.slug}`}
                className="premium-glass rounded-2xl overflow-hidden border border-border flex flex-col justify-between hover:border-primary/30 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/5 transition-all duration-300 group relative cursor-pointer w-full"
              >
                {blog.featured && (
                ...
                <div className="p-6 bg-muted/40 border-t border-border/40 flex flex-col gap-4 w-full">
                  ...
                  <span 
                    className="inline-flex items-center gap-1.5 text-xs font-bold text-foreground group-hover:text-primary transition-colors mt-2"
                  >
                    Read Full Article <ArrowRight className="h-3.5 w-3.5 transition-transform duration-300 group-hover:translate-x-1" />
                  </span>
                </div>
              </Link>
            </ScrollReveal>
